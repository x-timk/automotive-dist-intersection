...
Il protocollo \emph{discovery} è utilizzato da un veicolo in prossimità dell'incrocio per annunciare la propria presenza e conoscere gli altri veicoli interessati ad impegnare lo stesso.

I messaggi utilizzati nel protocollo sono i seguenti:
\begin{itemize}
\item \texttt{disc}: questo messaggio viene inviato in broadcast. Il payload deve contenere l'identità del veicolo mittente e la sua rotta.
\item \texttt{hello}: messaggio unicast inviato in risposta ad un disc dai soli veicoli in stato \texttt{DISCOVER}. Il payload deve contenere l'identità del veicolo mittente e la sua rotta.
\item \texttt{wait}: messaggio unicast inviato in risposta ad un disc dai veicoli in stato \texttt{ELECTION} \texttt{MASTER} \texttt{SLAVE} \texttt{CROSSING}. Il payload non contiene alcuna informazione.
\end{itemize} 

Un veicolo non appena entra nello stato \texttt{DISCOVER} invia un messaggio disc in broadcast a tutti i veicoli nelle vicinanze.
\begin{itemize}
\item I veicoli nello stato INQUEUE ignorano il messaggio. 
\item I veicoli in stato DISCOVER si salvano il mittente nella lista dei vicini e inviano un messaggio di hello come risposta, per segnalare la loro presenza in prossimità dell'incrocio.
\item I veicoli in stato ELECTION MASTER SLAVE CROSSING inviano come risposta un messaggio wait in modo da segnalare al mittente di aspettare perchè l'incrocio è già impegnato.
\end{itemize}

Il veicolo che ha inviato il messaggio disc può quindi ricevere:
\begin{itemize}
\item messaggi di \texttt{hello}: in questo caso si salva il riferimento del veicolo mittente tra i suoi vicini
\item messaggio \texttt{wait}: in questo caso il veicolo passa allo stato wait
\end{itemize}

La fase di discovery si considera conclusa al verificarsi dei seguenti eventi:
\begin{itemize}
...
Nei primi due casi il veicolo entrerà nello stato \texttt{ELECT}.
Nel terzo caso il veicolo ritenterà il \emph{discovery} una volta trascorso un determinato intervallo di tempo.
Nell'ultimo caso passerà direttamente nello stato \texttt{SLAVE}.
